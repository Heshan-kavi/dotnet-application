using System;
using Helloworld.Math;
using static System.Runtime.InteropServices.JavaScript.JSType;

namespace Variables
{
    class Program
    {
        static void Main(string[] args)
        {
            //c# basic types related functionalities
            //Helloworld.BasicTypesClass.BasicTypesRelatedFunctionalities();

            //check type conversions related functionalities
            //Helloworld.TypesClass.BasicTypeConversions();

            //using try and catch
            //try
            //{
            //    var stingValueEight = "1234";
            //    byte b = Convert.ToByte(stingValueEight);
            //    //Console.WriteLine(b);

            //}
            //catch (Exception)
            //{
            //    //Console.WriteLine("there is a type conversion issue with the values !!!");
            //}


            //operators related functionalities
            //Helloworld.OperatorsClass.OperatorsRelatedFunctionalities();

            //Checking non-primitve types in the C# related functionalities
            //Helloworld.NonPrimitiveClass.NonPrmitiveTypeRelatedFunctionalities();

            //testing array related functionalities
            //Helloworld.ArrayClass.BasicArrayRelatedFunctionalities();

            //strings and related things related functionalities
            //Helloworld.Strings.StringConcatentaion();

            //Enum related things related functionalities
            //Helloworld.EnumClass.EnumRelatedFunctionalities();

            //types related functionalities
            //Helloworld.TypesClass.TypeRelatedFunctionalities();

            //conditional statements related functionalities
            //Helloworld.ConditionalStatements.IfRelatedFunctionalities();
            //Helloworld.ConditionalStatements.SwitchRelatedFunctionalities();

            //iterational statments related functionalities
            //Helloworld.IterationalStatements.ForLoopRelatedFunctionalities();
            //Helloworld.IterationalStatements.ForEachRelatedFunctionalities();
            //Helloworld.IterationalStatements.DoWhileLoopRelatedFunctionalities();
            //Helloworld.IterationalStatements.WhileLooppRelatedFunctionalities();

            //random class related functionalities
            //Helloworld.RandomClass.RandomRelatedFunctionalities();

            //arrays related functionalities
            //Helloworld.ArrayClass.ArrayRelatedFunctionalities();

            //lists related functionalities
            //Helloworld.ListsClass.ListsRelatedFunctionalities();

            //date time related functionalities
            //Helloworld.DateTimeClass.DateTimeRelatedFunctionalities();

            //timestamp related functionalities
            //Helloworld.TimeSpanClass.TimeSpanRelatedFunctionalities();

            //strings related functionalities
            //Helloworld.Strings.FormatingFunctionalities();
            //Helloworld.Strings.OtherFunctionalities();

            //string builder
            //Helloworld.Strings.StringBuilderRelatedFunctionalities();

            //file and fileinfo related things
            //Helloworld.FileAndFileInfo.FileRelatedFunctionalities();
            //Helloworld.FileAndFileInfo.FileInfoRelatedFunctionalities();

            //directory and directoryinfo related things
            //Helloworld.FileAndFileInfo.DirectoryRelatedFunctionalities();
            //Helloworld.FileAndFileInfo.DirectoryInfoRelatedFunctionalities();

            //path related things
            Helloworld.PathClass.pathRelatedFunctionalities();
        }
    }
}

